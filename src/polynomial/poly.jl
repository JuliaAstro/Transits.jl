...
function Base.show(io::IO, ld::PolynomialLimbDark)
    return print(io, "PolynomialLimbDark(", ld.u_n, ")")
end
function Base.show(io::IO, ::MIME"text/plain", ld::PolynomialLimbDark)
    return print(io, "PolynomialLimbDark\n u_n: ", ld.u_n)
end
...
function compute_uniform(
    b::T, r; sqarea, r2=r^2, b2=b^2, fourbrinv=inv(4 * b * r)
) where {T}
    if b ≤ 1 - r
        flux = π * (1 - r2)
        kap0 = convert(T, π)
