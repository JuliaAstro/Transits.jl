...
function compute_Mn_coeffs(T, n_max; maxiter=100)
    return compute_Mn_coeffs!(zeros(T, 2, 4, maxiter), n_max)
end
...
function upwardM!(
    arr; sqbr, n_max, sqonembmr2, onemr2mb2, sqarea, k2, kap0, Eofk, Em1mKdm, kite_area2
)
    ...
    for n in 4:n_max
        arr[begin + n] =
            (
                2 * (n - 1) * onemr2mb2 * arr[begin + n - 2] +
                (n - 2) * sqarea * arr[begin + n - 4]
            ) / n
    end
    return arr
end
...
function downwardM!(
    ...
    for m in (n_max - 4):-1:4
        arr[begin + m] =
            ((m + 4) * arr[begin + m + 4] - 2 * (m + 3) * onemr2mb2 * arr[begin + m + 2]) *
            invsqarea / (m + 2)
    end
...
function Mn_four!(
    arr::AbstractVector{T}; kap0, sqbr, k2, Em1mKdm, onemr2mb2, kite_area2, Eofk, sqonembmr2
) where {T}
    # compute lowest four exactly
    if k2 ≤ 1 # eqn 77,79
        arr[begin] = kap0
...
function Nn_lower!(
    Nn::AbstractVector, Nn_coeff; Mn, onembpr2, sqonembmr2, n_max, k, k2, kwargs...
)
    Nn_series!(Nn, Nn_coeff; k2, sqonembmr2, n_max, k)

    for m in (n_max - 2):-1:2
        Nn[begin + m] =
            ((m + 4) * Nn[begin + m + 2] - Mn[begin + m + 2]) / (onembpr2 * (m + 2))
    end
